    ...
    function c_sinkron_to_gateoutlcl(){
        ...
        for($a=0;$a<count((array) $array_response);$a++){
            ...
            if($cek_reff_number == 1 &&  $cek_proses == 1){
                $hasil = $this->db_tpsonline->update('tbl_request_plp_out_kemasan',array('SENDING' => 'Y'),array('REF_NUMBER' => $str_replace_data));
        ...
        foreach($getdata->result_array() as $adata){
            ...
            if($excute_data->num_rows() > 0 ){
                ...
                foreach($excute_data->result_array() as $out_whs){
                    ...
                    $ID_CONSIGNEE = $this->db_tpsonline->query(" SELECT ID_CONSIGNEE FROM tbl_m_tps_consignee 
                        where CONSIGNEE_REPLACE = '".$CONSIGNEE_REPLACE."' ")->row()->ID_CONSIGNEE;
