...
class C_cont_stock_detail extends CI_Controller {
    ...
    function c_proses(){
        ...
        if($cont_status == "Empty" && $cbosort == "C"){
            ...
            $query.= " ifnull(b.beg - b.masuk, 0) as beg, ifnull(b.masuk, 0) as masuk, ifnull(b.keluar, 0) as keluar " ;
            $query.= " FROM t_m_principal as a " ;
            $query.= " left join view_cont_stock_detail_detail as b on a.code_principal=b.code_principal " ;
            $query.= " WHERE a. rec_id = '0' and b.reff_code is not null " ;
            $query.= " Group by a.code_principal, b.reff_code, b.cont_status, b.cont_condition " ;
            $query.= " Order by a.code_principal " ;
            $excute = $this->ptmsagate->query($query);
            ...
            foreach($excute as $row){
                ...
                $cek = $this->ptmsagate->get_where($nama_table_10,$where)->num_rows();
                $queryku[$a++] = $this->ptmsagate->last_query();
                ...
                if($cek == 0){
                    ...
                    //jika sudah ada
                    $this->ptmsagate->update($nama_table_10, array('masuk' => $row['masuk']), $where);
                    $queryku[$a++] = $this->ptmsagate->last_query();
            ...
            foreach($excute as $row){
                ...
                if($cek == 0){
                    ...
                    //jika sudah ada
                    $this->ptmsagate->update($nama_table_10, array('keluar' => $row['keluar']), $where);
                    $queryku[$a++] = $this->ptmsagate->last_query();
            ...
            $query = "";
            $query.= " CREATE TABLE test.".$nama_table_11." as ";
            $query.= " SELECT * from ".$nama_table_10." " ;
    ...
    function c_print(){
        ...
        if($cont_status == "Empty" && $cbosort == "C"){
            $path = APPPATH . 'modules/report_jasper/new/report_cont_stock_detail_sort_by_container_empty.jrxml'; 
            $path_subreport = "" ;
            ...
            $path = APPPATH . 'modules/report_jasper/new/report_cont_stock_detail_sort_by_date_empty.jrxml'; 
        }else if($cont_status == "Full" && $cbosort == "C"){
            $path = APPPATH . 'modules/report_jasper/new/report_cont_stock_detail_sort_by_container_full.jrxml'; 
        }else if($cont_status == "Full" && $cbosort == "D"){
            $path = APPPATH . 'modules/report_jasper/new/report_cont_stock_detail_sort_by_date_full.jrxml'; 
        }
